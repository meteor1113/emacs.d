...
(use-package emms
  ...
  (defun my/init-emms ()
    ...
    (or (featurep 'emms-setup)
        (when (and (require 'emms-setup nil t)
          ...
          (defun my/emms-mode-line-playlist-current-nondirectory ()
            "Format the currently playing song."
            (format emms-mode-line-format
                    (file-name-nondirectory
                     (emms-track-description
                      (emms-playlist-current-selected-track)))))
          (setq emms-mode-line-mode-line-function 'my/emms-mode-line-playlist-current-nondirectory)
          (setq emms-mode-line-titlebar-function 'my/emms-mode-line-playlist-current-nondirectory)
          (emms-mode-line 1)
  ...
  (defun my/init-emms--before-emms (&rest _)
    "Initial emms first."
    (my/init-emms))

  (defun my/init-emms--history-save-delete-empty (orig-fun &rest args)
  ...
  (advice-add 'emms :before #'my/init-emms--before-emms)
  (advice-add 'emms-history-save :around #'my/init-emms--history-save-delete-empty)

  :config
  (with-eval-after-load 'emms-playlist-mode
    (define-key emms-playlist-mode-map (kbd "SPC") 'emms-pause)
    (define-key emms-playlist-mode-map [double-mouse-1] 'emms-playlist-mode-play-current-track))
  )
